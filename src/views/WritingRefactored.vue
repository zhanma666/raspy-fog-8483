  ...
  <div class="writing-container">
    ...
    <header class="writing-header">
      ...
      <div class="header-right">
          ...
          <div class="info-item auto-save" v-if="writingConfig.isAutoSaving">
            <span class="label">自动保存中...</span>
          </div>
          <div class="info-item" v-else-if="writingConfig.lastSaveTime">
      ...
      <section class="writing-area">
        ...
        <div v-else-if="activeSection === 'background'" class="section-content">
          ...
          <textarea 
            ...
            v-model="paperContent.background"
            placeholder="请输入项目背景，包括项目的起源、目标、规模等信息"
            rows="20"
            @input="updateWordCount"
          ></textarea>
        </div>
        ...
        <div v-else-if="activeSection === 'problemAnalysis'" class="section-content">
          ...
          <textarea 
            class="content-textarea" 
            v-model="paperContent.problemAnalysis"
            placeholder="请分析项目中遇到的主要问题，包括技术、管理等方面"
            rows="20"
            @input="updateWordCount"
          ></textarea>
        </div>
        ...
        <div v-else-if="activeSection === 'solution'" class="section-content">
          ...
          <textarea 
            class="content-textarea" 
            v-model="paperContent.solution"
            placeholder="请描述针对问题提出的解决方案，包括技术架构、方法论等"
            rows="20"
            @input="updateWordCount"
          ></textarea>
        </div>
        ...
        <div v-else-if="activeSection === 'implementation'" class="section-content">
          ...
          <textarea 
            class="content-textarea" 
            v-model="paperContent.implementation"
            placeholder="请描述解决方案的实施过程，包括关键步骤、遇到的挑战及解决方法"
            rows="20"
            @input="updateWordCount"
          ></textarea>
        </div>
        ...
        <div v-else-if="activeSection === 'results'" class="section-content">
          ...
          <textarea 
            class="content-textarea" 
            v-model="paperContent.results"
            placeholder="请描述项目实施后的效果，包括技术指标、业务收益等"
            rows="20"
            @input="updateWordCount"
          ></textarea>
        </div>
        ...
        <div v-else-if="activeSection === 'conclusion'" class="section-content">
          ...
          <textarea 
            class="content-textarea" 
            v-model="paperContent.conclusion"
            placeholder="请总结项目经验教训，并对未来发展进行展望"
            rows="20"
            @input="updateWordCount"
          ></textarea>
        </div>
    ...
    <div class="modal-overlay" v-if="showSyllabusModal" @click="showSyllabusModal = false">
      <div class="modal-content syllabus-modal" @click.stop>
        <div class="modal-header">
          <h2>{{ examSyllabusRef?.title }}</h2>
          <button class="close-btn" @click="showSyllabusModal = false">&times;</button>
        </div>
        <div class="modal-body">
          <div v-if="examSyllabusRef" class="syllabus-content">
            <section class="syllabus-section">
              <h3>考试要求</h3>
              <ul>
                <li v-for="req in examSyllabusRef.requirements" :key="req.id">
            ...
            <section class="syllabus-section">
              ...
              <ul class="scoring-list">
                <li v-for="criterion in examSyllabusRef.scoringCriteria" :key="criterion.id">
            ...
            <section class="syllabus-section">
              ...
              <div class="structure-grid">
                <div 
                  v-for="section in examSyllabusRef.paperStructure" 
                  :key="section.id"
...
const examSyllabusRef = ref<ExamSyllabus | null>(getSyllabusBySubject('系统架构设计师') || null);
...
const initTemplates = () => {
  ...
  // 获取对应的考试大纲
  examSyllabusRef.value = getSyllabusBySubject(subject) || null;
};
